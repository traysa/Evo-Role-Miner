...
# -----------------------------------------------------------------------------------
def plotLogbookForMultiObjective(logbook, logbook_filename, title, info, objectives, saveAsPDF, saveAsSVG, saveAsPNG, showPNG):
    gen = logbook.select("gen")
    # Plot graphs
    ...
    for ax in plots:
        ...
        # Create graphs
        ax.set_xlabel("Generation", fontsize=16)
        ax.set_ylabel("Objective: "+objectives[i], fontsize=16)
...
# -----------------------------------------------------------------------------------------
def showFitnessInPlotForMultiObjective(results, generations, freq, filename, title, info, objectives, saveAsPDF, saveAsSVG, saveAsPNG, showPNG):
    print("\nCreate plot...")
    fig = plt.figure(figsize=(12, 8))
    ax = fig.add_subplot(111)
    # Plot a scatter graph of all results
    ...
    ax.set_xlim(0)
    ax.set_ylim(0)
    ax.set_xlabel('Objective: '+objectives[0], fontsize=16)
    ax.set_ylabel('Objective: '+objectives[1], fontsize=16)
    ax.tick_params(labelsize=14)
