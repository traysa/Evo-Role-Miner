    ...
    for ay in plots:
        ...
        for ax in ay:
            ...
            if individual == len(ay):
                ...
                ax.set_xlabel('Gen=' + str(results[p][2]) + '; Eval=' + str(results[p][0][0]) + ',' + str(
                    results[p][0][1]) + '; Roles=' + str(results[p][0][2]))
            individual += 1
            p = p + 1
    fig.tight_layout()
    plt.show()
...
def addBestIndividualToPlot(pop, generation):
    printPopulation(pop)
    bestInd = []
    ...
    for ind in pop:
        ...
        if (bestFit > fit):
            ...
            bestInd = ind
    if (bestInd != []):
        matrix = resolveChromosomeIntoMatrix(bestInd[0])
